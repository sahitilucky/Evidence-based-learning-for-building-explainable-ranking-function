...
def document_gen_probability(query_att_value_gen, product_att_values, product_tokens, tf_att_values):
    words = list(set(product_tokens.split(' ')))
...
'''
query_att_value_gen = pickle.load(open(OutputDirectory+'query_att_value_train_train_division_filtered.p','rb'))
with open(OutputDirectory + 'query_att_value_train_train_division_filtered.json', 'w') as fp:
...
unigram_probabilities = pickle.load(open(OutputDirectory + 'Unigram_model_probabilities_test_valid_filtered.p', 'rb'))
...
LTRtrainingfile = io.open(OutputDirectory+'ClickData_prob_features_four_components_test_valid_filtered.txt','w', encoding='utf-8') 
ranked_docs = {}
qid = 0
...
for query in rel_jud2:
    ...
    for productId in rel_jud2[query2]:
        ...
        if productId in document_query_word:
            ...
            #FeatureVector.append(str("{0:.6f}".format(query_gen_probability(document_query_word[productId], query))))
            probability3 = 0.0001*query_gen_probability(document_query_word[productId], query) 
            text += 'probability3 ' + str(alpha3_parameter[productId]) + ' ' + str(probability3/0.0001)+' '
            LTRtrainingfile.write('5:'+ str(alpha3_parameter[productId]).decode('utf-8')+' 6:' + str(query_gen_probability(document_query_word[productId], query)).decode('utf-8')+' ')
            LTRtrainingfile.write('7:'+ str(probability3).decode('utf-8')+' ')

        else:
            #FeatureVector.append(str("{0:.6f}".format(0.0)))
            probability3 = 0.0001*math.log(0.000000001)
            text += 'probability3 ' + str(0.0001) + ' ' + str(math.log(0.000000001))+' '
            LTRtrainingfile.write('5:0.0001 6:'+str(math.log(0.000000001)).decode('utf-8') + ' 7:'+str(probability3).decode('utf-8')+' ')
        try:
            print ('are you taking time?3')
            probability4 = alpha4_parameter[query]*unigram_probabilities[query][productId]
            #print ('came here')
            LTRtrainingfile.write('8:'+str(alpha4_parameter[query]).decode('utf-8')+' 9:' + str(unigram_probabilities[query][productId]).decode('utf-8')+' ')
            LTRtrainingfile.write('10:'+ str(probability4).decode('utf-8')+' ')
        except:
            print ('are you taking time?4')
            probability4 = (float(1)/float(100))*unigram_probabilities[query][productId]
            LTRtrainingfile.write('8:'+str(float(1)/float(100)).decode('utf-8')+' 9:' + str(unigram_probabilities[query][productId]).decode('utf-8')+' ')
            LTRtrainingfile.write('10:'+ str(probability4).decode('utf-8')+' ')
        
        probability = (alpha1_parameter*probability1) + (probability2) + (probability3) + probability4
        #ClickOutputfile.write((text+'\n'))
        
        try:
            probability = probability #+ alpha4_parameter[query]*add_prior(prior, productId) #- (0.5*negative_fb)
            LTRtrainingfile.write('11:' + str(add_prior(prior, productId)).decode('utf-8')+' ')
            LTRtrainingfile.write('12:'+ str(alpha4_parameter[query]*add_prior(prior, productId)).decode('utf-8')+' ')
        except:
            probability = probability #+ (float(1)/float(100))*add_prior(prior, productId)
            LTRtrainingfile.write('11:' + str(add_prior(prior, productId)).decode('utf-8')+' ')
            LTRtrainingfile.write('12:'+ str((float(1)/float(100))*add_prior(prior, productId)).decode('utf-8')+' ')
...
with open(OutputDirectory + 'search_log_test_valid_filtered_without_prior_2_2.txt', 'w') as output_file:
